...
exports.diagnosticsEnabled = diagnosticsEnabled;
exports.attachAudioMeta = attachAudioMeta;
exports.getAudioMeta = getAudioMeta;
exports.appendLineage = appendLineage;
exports.markAudioSpan = markAudioSpan;
exports.probePcm = probePcm;
exports.probeFloat32 = probeFloat32;
exports.probeWav = probeWav;
/**
...
const lineageWarned = new Set();
function diagnosticsEnabled() {
    return env_1.env.AUDIO_DIAGNOSTICS === true;
}
function attachAudioMeta(buffer, meta) {
    bufferMeta.set(buffer, meta);
    return meta;
}
function getAudioMeta(buffer) {
    return bufferMeta.get(buffer);
}
function appendLineage(meta, step) {
    const lineage = [...(meta?.lineage ?? []), step];
    const next = { ...meta, lineage };
    warnOnDuplicateTransforms(next, step);
    return next;
}
function markAudioSpan(event, meta) {
...
function probePcm(tag, pcm, meta = {}) {
    if (!diagnosticsEnabled())
        return;
    if (!pcm || pcm.length === 0)
        return;
...
function probeFloat32(tag, floats, meta = {}) {
    if (!diagnosticsEnabled())
        return;
...
function probeWav(tag, wavBytes, meta = {}) {
    if (!diagnosticsEnabled())
        return;
...
function analyzePcm16(buffer, meta) {
...
function analyzePcmu(buffer, meta) {
    const channels = meta.channels ?? 1;
    const sampleRate = meta.sampleRateHz ?? DEFAULT_SAMPLE_RATE;
...
function analyzeFloat32(floats, meta) {
    const channels = meta.channels ?? 1;
    const sampleRate = meta.sampleRateHz ?? DEFAULT_SAMPLE_RATE;
...
function analyzePcm16FromWav(wav, header, meta) {
...
function parseWavHeader(buffer) {
    if (buffer.length < 12)
        throw new Error('wav_header_too_small');
...
function emitProbe(tag, stats, meta) {
...
function clampSampleCount(sampleCount, sampleRate, channels) {
    if (sampleCount <= 0)
        return 0;
    if (sampleRate <= 0 || channels <= 0)
        return Math.min(sampleCount, 4000);
...
function fingerprintBuffer(buffer) {
    ...
    return (0, crypto_1.createHash)('sha1').update(slice).update(String(length)).digest('hex');
}
function resolveSpanKey(meta) {
    if (meta.callId)
...
function warnOnDuplicateTransforms(meta, step) {
    if (!diagnosticsEnabled())
        return;
    const lineage = meta.lineage ?? [];
    if (lineage.length < 2)
...
//# sourceMappingURL=audioProbe.js.map