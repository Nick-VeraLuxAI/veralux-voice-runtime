...
async function generateAssistantReply(input) {
    if (!env_1.env.BRAIN_URL) {
        const text = (0, defaultBrain_1.defaultBrainReply)({ transcript: input.transcript, tenantId: input.tenantId });
        log_1.log.info({
            event: 'brain_route',
    ...
    const url = buildBrainUrl(env_1.env.BRAIN_URL);
...
async function generateAssistantReplyStream(input, onToken) {
    if (!env_1.env.BRAIN_URL || !env_1.env.BRAIN_STREAMING_ENABLED) {
        return generateAssistantReply(input);
    }
    const streamUrl = buildBrainStreamUrl(env_1.env.BRAIN_URL);
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), env_1.env.BRAIN_TIMEOUT_MS);
