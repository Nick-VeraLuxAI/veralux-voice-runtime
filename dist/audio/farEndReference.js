...
exports.pushFarEndFrames = pushFarEndFrames;
exports.pullFarEndFrame = pullFarEndFrame;
exports.releaseFarEndBuffer = releaseFarEndBuffer;
exports.getFarEndFrameCount = getFarEndFrameCount;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const env_1 = require("../env");
const log_1 = require("../log");
const wavInfo_1 = require("./wavInfo");
...
function extractPcm16FromWav(wav) {
    const info = (0, wavInfo_1.parseWavInfo)(wav);
...
function resamplePcm16To16k(pcm, inputRateHz) {
    if (inputRateHz <= 0 || pcm.length === 0)
        return new Int16Array(0);
    if (inputRateHz === TARGET_SAMPLE_RATE_HZ)
        return pcm;
 ...
 */
function pushFarEndFrames(callControlId, wav, logContext) {
    try {
 ...
 */
function pullFarEndFrame(callControlId) {
 ...
 */
function releaseFarEndBuffer(callControlId) {
    const buf = buffersByCall.get(callControlId);
    if (buf && buf.frames.length > 0 && env_1.env.STT_DEBUG_DUMP_FAR_END_REF) {
        dumpFarEndToWav(callControlId, buf.frames);
    }
    const removed = buffersByCall.delete(callControlId);
    if (removed) {
        log_1.log.debug({ event: 'far_end_ref_released', call_control_id: callControlId }, 'far-end reference buffer released');
    }
}
...
function dumpFarEndToWav(callControlId, frames) {
    ...
    const outPath = path_1.default.join(dir, `far_end_ref_${safe}_${Date.now()}.wav`);
    try {
        fs_1.default.mkdirSync(dir, { recursive: true });
        fs_1.default.writeFileSync(outPath, Buffer.concat([header, pcm]));
        log_1.log.info({
        ...
        log_1.log.warn({ event: 'far_end_ref_dump_failed', call_control_id: callControlId, err: String(err) }, 'far-end reference dump failed');
    }
}
/**
 * Get current frame count for a call (for debugging/dumps).
 */
function getFarEndFrameCount(callControlId) {
    return buffersByCall.get(callControlId)?.totalFrames ?? 0;
}
//# sourceMappingURL=farEndReference.js.map