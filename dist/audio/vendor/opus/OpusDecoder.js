...
// @ts-nocheck
const WASMAudioDecoderCommon_1 = __importDefault(require("./WASMAudioDecoderCommon"));
const EmscriptenWasm_1 = __importDefault(require("./EmscriptenWasm"));
class OpusDecoder {
    constructor(_WASMAudioDecoderCommon, _EmscriptenWASM) {
        this._isWebWorker = _WASMAudioDecoderCommon && _EmscriptenWASM;
        this._WASMAudioDecoderCommon =
            _WASMAudioDecoderCommon || WASMAudioDecoderCommon_1.default;
        this._EmscriptenWASM = _EmscriptenWASM || EmscriptenWasm_1.default;
    ...
    decodeFrame(opusFrame) {
        if (!(opusFrame instanceof Uint8Array))
            throw Error(`Data to decode must be Uint8Array. Instead got ${typeof opusFrame}`);
        this._input.set(opusFrame);
        const samplesDecoded = this._common.wasm._opus_frame_decode_float_deinterleaved(this._decoder, this._inputPtr, opusFrame.length, this._leftPtr, this._rightPtr);
    ...
    decodeFrames(opusFrames) {
        ...
        return this._WASMAudioDecoderCommon.getDecodedAudioConcat([left, right], samples, 48000);
    }
}
