...
function prepareAmrWbPayload(payload) {
    const getHexPrefix = (buf, len = 16) => buf.subarray(0, len).toString('hex');
    ...
    const isValidOctetAlignedToc = (toc) => {
        ...
        if (!(ft === 15 || ft === 9 || (ft >= 0 && ft <= 8)))
            return { ok: false };
        return { ok: true, ft, q };
    };
    const octetAlignedNoCmrMatch = () => {
        if (length < 2)
            return false;
        const tocInfo = isValidOctetAlignedToc(payload[0]);
        if (!tocInfo.ok || tocInfo.ft === undefined)
            return false;
    ...
    const octetAlignedWithCmrMatch = () => {
        if (length < 3)
            return false;
    ...
    const looksLikeBeTocAt = (buf, bitOffset) => {
