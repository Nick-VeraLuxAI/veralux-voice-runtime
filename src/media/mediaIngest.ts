...
export class MediaIngest {
  ...
  private guessDumpKind(raw: Buffer): 'amrwb_magic' | 'amrwb_toc_like' | 'wav_riff' | 'unknown' {
    if (looksLikeAmrWbMagic(raw)) return 'amrwb_magic';
    if (looksLikeTelnyxNoCmrToc33(raw)) return 'amrwb_toc_like';
