import { log } from '../log';
import { appendLineage, attachAudioMeta, getAudioMeta } from '../diagnostics/audioProbe';
...
export interface PlaybackPipelineOptions {
  ...
  enableRmsNormalize?: boolean;
  logContext?: Record<string, unknown>;
}

export interface PlaybackPipelineResult {
  audio: Buffer;
  applied: boolean;
}
...
function highPassFilter(samples: Float32Array, sampleRateHz: number): Float32Array {
...
function createLowPassBiquad(sampleRateHz: number, cutoffHz: number, q: number): BiquadCoeffs {
...
function createBandPassBiquad(sampleRateHz: number, centerHz: number, q: number): BiquadCoeffs {
...
function applyDeEsser(samples: Float32Array, sampleRateHz: number): Float32Array {
  ...
  for (let i = 0; i < samples.length; i += 1) {
    ...
    if (env > DEESSER_THRESHOLD) {
      ...
      gain = outEnv / env;
      if (gain < DEESSER_MIN_GAIN) gain = DEESSER_MIN_GAIN;
    }
...
function applyRmsNormalize(samples: Float32Array): boolean {
...
function encodeWavFromPcm16(samples: Int16Array, sampleRateHz: number): Buffer {
...
function resolveLowpassCutoff(inputSampleRate: number, outputSampleRate: number): number {
...
interface TelephonyMasteringOptions {
  enableHighpass: boolean;
  enableRmsNormalize: boolean;
}

interface TelephonyMasteringResult {
  pcm16: Int16Array;
  appliedHighpass: boolean;
  appliedDeEsser: boolean;
  appliedLowpass: boolean;
  appliedResample: boolean;
  appliedLimiter: boolean;
  appliedRmsNormalize: boolean;
  lowpassCutoffHz: number;
  outputSampleRateHz: number;
}

function processTelephonyTTSInternal(
  pcm: Float32Array | Int16Array,
  inputSampleRate: number,
  outputSampleRate: number,
  options: TelephonyMasteringOptions,
): TelephonyMasteringResult {
...
export function processTelephonyTTS(
  pcm: Float32Array | Int16Array,
  inputSampleRate: number,
  outputSampleRate: number,
): Int16Array {
...
export function runPlaybackPipeline(
  input: Buffer,
  options: PlaybackPipelineOptions,
): PlaybackPipelineResult {
  try {
    ...
    meta = appendLineage(meta, 'decode:wav->pcm16le');
    ...
    if (result.appliedHighpass) {
      meta = appendLineage(meta, 'filter:highpass_100hz');
    }
    if (result.appliedDeEsser) {
      meta = appendLineage(meta, `deesser:${DEESSER_LOW_HZ}-${DEESSER_HIGH_HZ}hz`);
    }
    if (result.appliedLowpass) {
      meta = appendLineage(meta, `filter:lowpass_${Math.round(result.lowpassCutoffHz)}hz`);
    }
    if (result.appliedResample) {
      meta = appendLineage(meta, `resample:${header.sampleRateHz}->${result.outputSampleRateHz}`);
    }
    if (result.appliedLimiter) {
      meta = appendLineage(meta, 'limiter:soft');
    }
    if (result.appliedRmsNormalize) {
      meta = appendLineage(meta, 'normalize:rms');
    }

    meta = appendLineage(meta, 'wrap:wav');
    meta = {
      ...meta,
      format: 'wav',
